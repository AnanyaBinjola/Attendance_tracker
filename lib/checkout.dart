...
// import 'package:login_app/profile.dart'; // <--- UPDATED: Import ProfileDisplayScreen
...
//   // Reload profile data when returning from ProfileDisplayScreen
...
//                   MaterialPageRoute(
//                     builder: (context) => ProfileDisplayScreen( // <--- Changed to ProfileDisplayScreen
//                       userMobile: widget.userMobile, 
//                       userName: widget.userName,
//                     ),
//                   ),
//                 ).then((_) => _onProfileScreenClosed()); // Restart timer and reload data when returning
//               },
//             ),
...
class _CheckoutSuccessScreenState extends State<CheckoutSuccessScreen> {
  ...
  Future<void> _loadUserProfile() async {
    final dbHelper = DatabaseHelper();
    // Corrected to use the 'mobile' key
    final user = await dbHelper.getUser(widget.userMobile, widget.userName);
    ...
    if (mounted) {
      setState(() {
        if (user != null) {
          _profileName = user['name'] ?? 'N/A';
          _profileMobile = user['mobile'] ?? 'N/A'; // Corrected key
          _profileEmail = user['email'] ?? 'N/A';
        } else {
          _profileName = widget.userName;
          _profileMobile = widget.userMobile;
  ...
  // This is the new method to show the logout confirmation dialog
        ...
        return AlertDialog(
          ...
          actions: <Widget>[
            TextButton(
              ...
              onPressed: () {
                // Clicking 'Go back' closes the dialog and stays on the current screen
